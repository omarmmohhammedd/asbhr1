        ...
        <div style={{minHeight:'60vh'}} className='w-full flex justify-center items-center md:items-start'>
                <div className='md:w-1/2 w-2/3  py-5 shadow-md my-2 h-fit ' style={{border:'1px solid #eee'}}>
                    <div className='flex w-full items-center gap-x-3  border-b-2  pb-3' style={mode==='ar'?{borderRight:'2px solid #74f374 ',paddingRight:'5px'}:{borderLeft:'2px solid #74f374 ',paddingLeft:'5px'}} dir={`${mode === 'ar' ?'rtl':'ltr'}`}>
                        ...
                        <span className='md:text-sm text-xs font-semibold'>{checkMode('Verify Order','التحقق من الطلب').word}</span>
                    </div>
                    <form className='flex flex-col w-full  items-center pt-5 gap-y-5' onSubmit={handleSubmit}>
                        <div className='flex gap-x-5  text-gray-500 lg:text-sm md:font-semibold text-xs justify-center items-center otp-desc'>
                                ...
                                <span >{checkMode('Wait for the timer to end before entering the code','انتظر  نهايه العداد قبل ادخال الرمز ').word}</span>
                        </div>
                        <input className='md:w-1/2 w-2/3 outline-none  rounded-sm px-2 py-1 md:text-sm otp-desc' style={{border:'1px solid #eee'}} dir={`${mode === 'ar' ?'rtl':'ltr'}`} placeholder={`${mode ==='ar' ? 'ادخل رمز التحقق المرسل الي جوالك' : 'Send OTP That Sent To Your Phone'}`} required type='text' onChange={(e)=>setOtp(e.target.value.replace(/\D/g, ''))}  inputMode="numeric"   value={otp}/>
